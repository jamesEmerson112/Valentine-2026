...
export function generateObstacles(
  ...
  while (obstacles.length < count && attempts < maxAttempts) {
    ...
    const w = OBSTACLE_SIZE_MIN + Math.random() * (OBSTACLE_SIZE_MAX - OBSTACLE_SIZE_MIN)
    const h = OBSTACLE_SIZE_MIN + Math.random() * (OBSTACLE_SIZE_MAX - OBSTACLE_SIZE_MIN)
    ...
    if (rectsOverlap(candidate, gardenBox, OBSTACLE_GARDEN_CLEARANCE)) continue

    // Check flower clearance
    let tooCloseToFlower = false
    for (const flower of flowers) {
      if (pointInRect(flower.x, flower.y, candidate, OBSTACLE_FLOWER_CLEARANCE)) {
    ...
    for (const existing of obstacles) {
      ...
      if (rectsOverlap(candidate, existingRect, 10)) {
        overlaps = true
        break
      }
    }
